...
/**
 * Get scoreboard score in key
 * @param {String | Object} key
 * @param {String} element
 **/
Score.prototype.member_score = function(conditions, callback) {
    this.query = new Query(conditions, this).type('member_score');
    return this.query.find(callback);   
};
...
Query.prototype.member_score = function(callback) {
    var keys = [];
    ...
    if(this.keys.length == 1) {
        var db = this.score.client;
        db.zscore(keys[0], this.conditions.element, function(err, res) {
            callback(err, res);
        });
        
    } else {
        callback(new Error("only one key can be added"));
    }
};